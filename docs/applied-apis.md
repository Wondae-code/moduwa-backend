...
| # | API | 기관 | 엔드포인트 | 비고 |
|---|---|---|---|---|
| 1 | 식품_관광식당 조회서비스 | 행안부 | `https://apis.data.go.kr/1741000/tourist_restaurants` | 일 10,000 · 수집 안 함(아래 ⚠️) |
...
| # | API | 수집 스크립트 | 적재 |
|---|---|---|---|
| 1 | 식품_관광식당 | (수집 안 함) | 0건 — 수집해 봤으나 제외 결정, 아래 ⚠️ 참고 |
...
> ⚠️ **식품_관광식당 — 수집하지 않기로 결정했다(2026-09-05).**
>
> 한 번 수집해 실물을 확인한 뒤 내린 판단이다. 규모와 지역 편중이 우리 용도에 맞지 않는다:
> - **영업중 142곳뿐**이다(526 레코드 → 사업장 218곳 → 영업중 142곳). 우리는 이미 `kor_poi` 에
>   음식점 **16,653곳**을 갖고 있다.
> - **지역이 몰려 있다** — 경남 67 + 서울 45 = 112곳(79%). 부산·인천·대구·울산·세종·충북·제주는
>   0곳이고 강원 1곳·충남 1곳이다. 소도시 식당 부족을 메우려던 원래 목적에 맞지 않는다
>   (가평 0 · 평창 0 · 영월 0 · 함양 0 — 같은 지역에 `kor_poi` 는 124 · 379 · 43 · 32곳).
> - 기존 데이터와 중복은 13%(18/142)로 낮아 **새 장소 자체는 늘지만**, 늘어나는 곳이 이미
>   식당이 충분한 지역이라 코스 품질에 기여하지 못한다.
>
> 다시 수집하기로 하면 아래 세 함정을 그대로 만난다(실측으로 확인한 내용이라 남겨 둔다).
>
> ⚠️ **이 API 는 다른 API 와 두 가지가 다르다.**
...
> 의 일정한 오프셋이 나왔다. 주소를 카카오로 지오코딩하면 영업중 142곳 중 **141곳**을 얻는다
> (폴백 순서: 도로명 → 광역명 제거 → 지번 → 건물명 → 상호. 실패 1곳은 카카오에 없는 옛 지번).
>
> 구현은 커밋 `6617a9b` 에 있다 — 되살리려면 그 커밋의 `sql/050_*.sql` 과
> `src/ingest-restaurants.ts` 를 가져오면 된다.
