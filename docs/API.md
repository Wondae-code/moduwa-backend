...
### POST /v1/auth/google · POST /v1/auth/apple — 소셜 로그인
...
| `nickname` | — | **애플용.** 애플은 이름을 토큰에 담지 않고 최초 인증 응답에서만 주므로, 앱이 그때 받은 이름을 여기 싣는다. 새 계정의 초기 닉네임으로만 쓰인다 |

- 서버는 토큰의 **서명·발급자·만료**를 프로바이더 JWKS 로 검증하고, `aud` 가 **우리 앱의
  클라이언트 ID**(`GOOGLE_CLIENT_IDS` / `APPLE_CLIENT_IDS`)인지 확인합니다.
...
- **카카오·네이버 로그인** — 스키마와 계정 처리는 준비돼 있습니다(검증 함수만 늘리면 됩니다).
...
| `infant` | 영유아 | 2,998 | 878 | 1,076 | 264 | 0 |
...
> 그래서 고령자 친화(`elderly`)를 보내도 결과가 좁혀지지 않는다 — 관광공사 원본에 그 축이 없다.
