...
- `linked`·`passwordReset` 은 소셜 로그인에서 의미가 있습니다(아래 "같은 이메일, 다른 로그인 방식").
  이메일 가입에서는 항상 `false` 입니다.
...
**`email_taken` 에는 `providers` 가 함께 옵니다** — 그 주소를 쓰는 계정의 로그인 방식입니다.
```json
{ "error": "email_taken", "message": "구글 로그인으로 가입된 이메일입니다. 그 방식으로 로그인해주세요.", "providers": ["google"] }
```
- `providers` 에 `"email"` 이 있으면 이메일 로그인 화면으로, 없으면(`["google"]` 처럼) **그 소셜
  버튼으로** 안내하세요. 구글로만 가입한 사람은 비밀번호가 없어서 이메일 로그인이 항상 실패합니다.
- 소셜로 가입된 주소에 이메일 가입으로 비밀번호를 붙여 주지는 **않습니다.** 이메일 소유를
  증명하지 않은 사람이 남의 계정의 비밀번호를 정하게 되기 때문입니다. 반대 방향(구글 → 기존
  이메일 계정)만 자동으로 이어집니다(아래 소셜 로그인 참고).
...
| `newAccount` | — | 카카오 `link_required`(409) 를 받은 뒤 "새 계정으로 시작" 을 골랐을 때만 `true`. 그 외에는 무시된다 |
...
- 계정은 **`(provider, subject)`** 로 정합니다. 이메일은 바뀔 수 있어서 계정의 키가 아닙니다.
- 애플은 이메일을 **최초 1회만** 줍니다. 계정 대표 이메일은 비어 있을 때만 채우므로
  두 번째 로그인에서 `null` 이 와도 지워지지 않습니다.

#### 같은 이메일, 다른 로그인 방식 — 자동 연결

이메일로 가입한 사람이 나중에 같은 주소의 구글로 로그인하면 **새 계정을 만들지 않고 기존
계정에 구글 신원을 붙입니다.** 응답은 **200** 이고 `created: false, linked: true` 입니다.
앱은 온보딩을 띄우지 말고 "기존 계정에 연결했습니다" 정도만 알려 주면 됩니다.

붙이는 조건은 하나입니다 — **프로바이더가 이메일 소유를 확인해 줬을 것**(`email_verified`).
구글·애플은 그렇고, 카카오는 확인 수단이 없어 자동으로는 절대 붙이지 않습니다. 대신 **묻습니다**
(아래 "카카오 — `link_required`").

| 기존 계정 상태 | 결과 |
|---|---|
| 이메일 인증(6자리 코드)을 마쳤다 | 붙인다. `passwordReset: false` |
| 이메일 인증을 안 했다 | 붙이되 **이메일 비밀번호를 지우고 모든 세션을 끊는다.** `passwordReset: true` |

두 번째 줄이 보안 장치입니다. 남의 주소로 이메일 가입만 해 두고(인증은 못 합니다) 비밀번호를
쥔 채 기다리는 **선점 탈취** 패턴이 있습니다. 주소의 진짜 주인이 구글로 들어오는 순간 그
비밀번호를 무효화하면 선점한 쪽은 잘리고, 진짜 주인은 구글로 쓰거나 **비밀번호 찾기**(그
주소로 코드가 갑니다)로 새 비밀번호를 정하면 됩니다. `passwordReset: true` 를 받으면 앱은
"이메일 비밀번호는 다시 설정해야 합니다" 를 한 번 알려 주세요. 서버는 같은 시점에 **그 주소로
알림 메일**도 보냅니다 — 앱 화면은 방금 소셜로 들어온 사람만 보는데, 그 소셜 계정이 탈취된
것이었다면 원래 주인이 알 길이 메일뿐이기 때문입니다.

- 연결할 때 **기존 계정의 닉네임·프로필 사진·무장애 항목은 그대로**입니다. 요청에 실린
  `accessFeatures`·`nickname` 은 무시됩니다(로그인과 같은 규칙).
- 애플의 "이메일 가리기"(비공개 릴레이 주소)는 기존 주소와 같을 수 없으므로 연결되지 않습니다.
  그 사람은 별도 계정이 됩니다. 로그인한 뒤 설정에서 붙일 수 있습니다(아래 "연결된 계정").
- 같은 토큰으로 요청이 겹치면 **409 `conflict`** — 다시 시도하면 두 번째는 그냥 로그인됩니다.

#### 카카오 — 같은 이메일의 계정이 있으면 `link_required`(409)

카카오 이메일이 기존 계정의 주소와 같으면 서버는 새 계정을 만들지 않고 **409 를 돌려주고 앱이
묻게 합니다.** 말없이 별도 계정을 만들면 사용자는 후기·플랜이 빈 계정을 보게 되기 때문입니다.
```json
{ "error": "link_required",
  "message": "이 이메일로 가입된 계정이 있습니다(이메일 로그인). 기존 계정에 연결하거나 새 계정으로 시작할 수 있습니다.",
  "providers": ["email"] }
```
앱은 다이얼로그 하나를 띄웁니다. **카카오 `idToken` 은 버리지 말고 들고 있으세요.**

| 사용자가 고른 것 | 앱이 할 일 |
|---|---|
| **기존 계정에 연결** | `providers` 의 방식으로 로그인(이메일이면 비밀번호 입력, 구글이면 구글 버튼) → 세션을 얻은 뒤 들고 있던 카카오 `idToken` 을 **`POST /v1/auth/me/identities`** 로 보낸다(아래). 이후 카카오로 바로 로그인된다 |
| **새 계정으로 시작** | 같은 요청에 **`newAccount: true`** 를 붙여 다시 보낸다 → **201**. 별도 계정이 생기고 그 계정의 `email` 은 `null` 로 시작한다(주소는 기존 계정 것이라 올리지 않는다) |
| 취소 | 아무것도 하지 않는다. 계정은 만들어지지 않았다 |

`newAccount` 는 이 409 를 받은 뒤에만 의미가 있습니다. 구글·애플처럼 검증된 이메일에는 영향이
없고(자동 연결이 우선), 겹치는 계정이 없을 때도 무시됩니다.
...
오류: `missing_idToken`(400) · `invalid_token`(401) · `social_not_configured`(503) · `too_many_attempts`(429) · `link_required`(409) · `conflict`(409)

### GET · POST · DELETE /v1/auth/me/identities — 연결된 계정  🔒

로그인한 상태에서 다른 로그인 방식을 붙이고 뗍니다. 지금 쓰이는 곳은 **카카오 `link_required`
의 "기존 계정에 연결"** 입니다(위). 설정 화면("연결된 계정")을 두면 애플 "이메일 가리기"처럼
주소가 달라 자동 연결이 안 되는 경우도 덮을 수 있지만, 그 화면은 아직 계획에 없습니다.
근거는 이메일이 아니라 **지금 세션이 이 계정의 주인**이라는 사실이라 이메일이 달라도 붙습니다.

**GET** → 이 계정에 붙은 방식 목록.
```json
{ "identities": [
  { "provider": "email",  "email": "me@example.com", "linkedAt": "2026-08-01T02:00:00Z" },
  { "provider": "kakao",  "email": "me@kakao.com",   "linkedAt": "2026-09-12T02:32:25Z" } ] }
```

**POST** `{provider, idToken, authorizationCode?}` — `provider` 는 `google`·`apple`·`kakao`.
`idToken` 은 소셜 로그인과 같은 토큰이고, 애플은 `authorizationCode` 도 함께 보내세요(폐기용).
→ **200** `{ linked, alreadyLinked, identities, author }`. 같은 소셜 계정을 다시 붙이면 오류가
아니라 `alreadyLinked: true` 입니다.

- 계정에 대표 이메일이 없고(카카오로 시작한 계정) 붙이는 쪽에 이메일이 있으면, **그 주소를 다른
  계정이 쓰지 않을 때만** 대표 이메일로 올립니다. 구글·애플처럼 검증된 주소면 인증 표시도 찍힙니다.
  `author` 에 반영되어 옵니다.
- 프로바이더당 하나만 붙습니다(구글 계정 두 개는 안 됩니다).

| 오류 | 뜻 |
|---|---|
| `identity_in_use`(409) | 그 소셜 계정이 **이미 다른 모두와 계정**에 붙어 있습니다. 합치지 않습니다 — 후기·플랜의 주인을 옮기는 일이라 별도 설계가 필요하고, 조용히 하면 "카카오를 눌렀더니 다른 계정이 흡수됐다"가 됩니다. "그 계정으로 로그인해주세요"로 안내하세요 |
| `provider_already_linked`(409) | 이 계정에 같은 프로바이더가 이미 붙어 있습니다. 먼저 해제해야 합니다 |
| `invalid_provider`·`missing_idToken`(400) · `invalid_token`(401) · `social_not_configured`(503) · `too_many_attempts`(429) | 소셜 로그인과 같습니다 |

**DELETE** `/v1/auth/me/identities/:provider` — `email` 도 뗄 수 있습니다(소셜만 남기고 싶은 사람).
→ **200** `{ identities }`. 애플을 떼면 애플 쪽 토큰도 폐기해 설정의 "Apple 로 로그인" 목록에서
우리 앱이 사라집니다.

| 오류 | 뜻 |
|---|---|
| `last_identity`(409) | **마지막 남은 로그인 방식은 뗄 수 없습니다.** 떼면 이 계정에 다시 들어올 길이 없습니다 |
| `not_linked`(404) | 붙어 있지 않은 방식 |
| `invalid_provider`(400) | 알 수 없는 값 |
...
- ⚠️ **`sort`를 명시하면 그쪽이 이깁니다.** `liked`가 무조건 이기게 하면 준 `sort`가 조용히
  ...
  `liked`는 **기본 정렬만** 바꿉니다
**`likedAt`** — `liked=true`일 때만 항목에 실립니다. 값은 정렬에 쓰는 것과 **같은** 시각입니다.

```jsonc
{ "id": 15, "likedAt": "2026-09-06T20:47:21Z", ... }
```

`GET /v1/posts?liked=true` 항목에도 같은 이름·같은 형식으로 실립니다. 두 목록을 `likedAt`
내림차순으로 합치면 **좋아요를 누른 순서 하나의 목록**이 됩니다 — 저장 탭이 게시글과 후기를
한 줄로 보여 줄 수 있는 기준입니다.

- ⚠️ **`createdAt`으로 섞지 마세요.** 그건 글이 쓰인 시각이라, 옛날 글에 어제 누른 하트가 맨
  아래로 갑니다
- `liked=true`가 아닌 목록에는 **필드 자체가 없습니다.** 좋아요 목록에서만 뜻이 있는 값이라
  늘 실으면 `null`이 무슨 뜻인지 갈립니다(안 눌렀다? 다른 목록인가?)
- `sort`를 함께 줘도 실립니다 — 정렬이 아니라 `liked`에 걸려 있습니다
...
### DELETE /v1/reviews/:reviewId — 후기 삭제 🔒

본인이 쓴 후기만 지웁니다. 성공하면 `204`(본문 없음).

| 상황 | 응답 |
|---|---|
| 내 후기 | `204` |
| **남의 후기** | `404` — `403`이 아닙니다 |
| 없는 id | `404` |
| 숫자가 아닌 id | `400` |
| 비로그인 | `401` |

- ⚠️ **남의 후기에 `403`이 아니라 `404`를 주는 이유**: `403`은 "그 후기가 있다"를 알려 줍니다.
  있는지 없는지도 남의 일입니다(게시글 삭제와 같은 판단)
- **댓글·좋아요·태그는 함께 사라집니다**(FK cascade). 신고 내역은 남습니다 — 운영 화면이
  "대상이 지워져 내용을 볼 수 없습니다"로 표시합니다
- **첨부 사진은 아무 데서도 안 쓸 때만 파일에서 지워집니다.** 파일 이름이 사진 내용의
  sha256이라 **같은 사진은 한 파일을 공유합니다** — 다른 사람이 올린 같은 사진, 내 게시글,
  내 프로필 사진이 같은 파일을 가리킬 수 있어, 하나라도 걸리면 파일을 남깁니다
- 후기 **수정**은 없습니다. 지우고 다시 쓰는 것이 유일한 방법입니다
...
같은 조건인 사람의 후기가 더 믿을 만하다 — 그 정보를 **프로필이 아니라 후기에** 붙입니다.

`GET /v1/review-tags` 가 두 종류를 함께 줍니다. `?kind=place` 또는 `?kind=visitor` 로 한쪽만 받을 수 있습니다.

| `kind` | 뜻 | 예 | 쓰이는 곳 |
|---|---|---|---|
| `place` | **장소** 평가 | `음식이 맛있어요` | 후기 뱃지 · **요약 막대 집계** |
| `visitor` | **방문자** 조건 | `휠체어로 방문했어요` | 후기 뱃지 · `visitorTag` 필터 |

작성은 기존과 같습니다 — `POST /v1/reviews` 의 `tags` 배열에 두 종류를 섞어 보내면 됩니다.

```
GET /v1/reviews?contentId=125984&visitorTag=visit_wheelchair
```
...
- 앱은 프로필 값을 보고 작성 화면에서 해당 항목을 **미리 체크해 제안**할 수 있습니다(코드가 다섯 축과 1:1). 저장은 후기 쪽에만 됩니다
- 태그 개수 상한은 카탈로그 전체 수(**13개**)입니다

## 민감정보 동의 (접근성 특성)

`accessFeatures`는 장애 관련 정보라 **민감정보**입니다. 개인정보보호법은 다른 항목과 **분리된
별도 동의**를 요구하고, 동의를 받은 사실을 **처리자가 입증**하도록 합니다. 그래서 서버가 동의
시각과 고지문 버전을 함께 저장합니다(050).

### 보내는 쪽
`accessFeatures`를 **비어 있지 않게** 보낼 때는 `sensitiveConsent: true`를 함께 보내야 합니다.
해당 라우트: `POST /v1/auth/email/sign-up`, `POST /v1/auth/{google|apple|kakao}`, `PATCH /v1/auth/me`.

```jsonc
{ "accessFeatures": ["wheelchairAccessible"], "sensitiveConsent": true }
```

| 상황 | 결과 |
|---|---|
| 값 있음 + `sensitiveConsent: true` | 저장 · 동의 시각·버전 기록 |
| 값 있음 + 동의 없음 | **400** `sensitive_consent_required` |
| **빈 배열** `[]` | 동의 없이도 통과 — **철회**입니다. 값과 동의 기록을 함께 지웁니다 |
| 키 자체를 안 보냄 | 아무것도 바꾸지 않음(온보딩 미완과 구분) |

- ⚠️ **빈 배열을 막지 않는 이유** — 지우는 것까지 동의를 요구하면 이용자가 자기 정보를 뺄 수 없습니다
- ⚠️ **가입 경로도 같은 규칙**입니다. 가입만 열어 두면 가드가 무의미합니다

### 받는 쪽 — `needsSensitiveConsent`
로그인·가입·`GET /v1/auth/me` 응답에 실립니다.

**`true`면 앱이 동의를 다시 받아야 합니다.** 접근성 특성은 있는데 동의 기록이 없다는 뜻입니다 —
동의를 안 받았다는 게 아니라 **기록을 시작하기 전에 등록한 계정**입니다(prod 8건). 다시 물어서
동의하면 `sensitiveConsent: true`와 기존 값을 그대로 `PATCH`하고, 거부하면 `accessFeatures: []`로
지웁니다.

### 동의 화면에 넣어야 하는 것 (법정 4항목)
| 항목 | 내용 |
|---|---|
| 수집 항목 | 접근성 특성(휠체어 이용, 시각·청각, 유아 동반, 고령자 관련 편의 정보) |
| 수집·이용 목적 | 무장애 여행 코스 추천 및 접근성 정보 표시 |
| 보유·이용 기간 | 회원 탈퇴 또는 이용자가 삭제할 때까지 |
| 거부 권리·불이익 | 동의하지 않아도 모든 기능 이용 가능, 불이익 없음 |

형식 요건: **다른 동의와 분리된 체크박스** · **기본값 해제** · **[선택] 표시**.

고지문을 고치면 `SENSITIVE_NOTICE_VERSION`(accounts.ts)도 함께 올립니다.

## 장소 공유 링크

### GET /p/:contentId — 장소 공유 대체 페이지
`https://moduwa.app/p/{contentId}` — 앱의 "공유하기"가 만드는 링크입니다. **앱이 설치돼 있으면
이 페이지는 뜨지 않고 iOS 가 앱으로 보냅니다**(`/i/:code` 와 같은 구조). 여기 도달했다는 건 앱이
없거나 카톡 인앱 웹뷰로 열었다는 뜻입니다.

- `contentId` 는 **숫자만** 통과합니다. 아니면 404 — URL 파라미터를 화면에 되돌리지 않습니다
- 없는 장소는 **200**에 "찾을 수 없는 장소" 안내입니다(404 가 아닙니다 — 카톡 카드가 깨지지 않게)
- 내용: 장소 이름 · 주소 · 대표 사진 · 무장애 항목 칩 · `moduwa://p/{contentId}` 「앱에서 열기」
- `APP_STORE_URL` 이 설정돼 있으면 「앱 받기」 버튼이 함께 나옵니다(비어 있으면 숨김)
- 무장애 정보를 표시하므로 **"한국관광공사 TourAPI" 출처를 페이지에 답니다**(표출 시 출처 표시 의무)

**OG 태그**를 넣습니다 — 카톡에서 카드로 보이느냐 파란 글씨로 보이느냐가 여기서 갈립니다.
장소를 못 찾은 경우에도 최소 OG(제목·설명)를 넣어 카드가 깨지지 않게 합니다.

- ⚠️ **삽입 값은 전부 HTML 이스케이프합니다.** 장소 이름에 `&`가 들어오는 경우가 실제로 있고(`사선대관광지&조각공원`), 이스케이프하지 않으면 속성값이 깨져 주입 경로가 됩니다

### AASA 에 등록된 경로
`/.well-known/apple-app-site-association` 의 `components` 에 `/i/*`(플랜 초대)와 `/p/*`(장소 공유)가
있습니다. `APPLE_APP_SITE_IDS` 가 비면 라우트 자체가 404 입니다 — 애플 CDN 이 "연결 없음" 상태를
캐시하는 것보다 낫습니다.

- ⚠️ **경로를 추가하면 애플 CDN 캐시 때문에 이미 설치된 기기는 앱을 다시 받아야 반영됩니다.**
  새 경로는 앱 배포보다 **먼저** 서버에 올려 두세요

## 계정 삭제 · 차단 (앱스토어 심사 필수)

### DELETE /v1/auth/me — 계정 삭제  🔒
성공 시 **204**. 앱은 로컬 세션을 지우고 비로그인 상태로 돌아갑니다. **되돌릴 수 없습니다.**

**작성자만 익명화하고 콘텐츠는 남깁니다.** 함께 쓴 대화가 통째로 사라지면 지운 사람이 아니라
남은 사람의 화면이 깨집니다 — 답글만 남은 스레드, 멤버가 사라진 공유 플랜, 후기 수가 줄어드는 장소.

| 지워지는 것 | 남는 것 |
|---|---|
| 이메일·비밀번호·프로필 사진 | 게시글·후기·댓글 본문과 사진 |
| 소셜 신원(구글·애플·카카오) 전부 | 신고 기록(운영 자료) |
| 세션 토큰 전부 · 기기 바인딩 | |
| **기기 토큰** — 남으면 탈퇴한 사람 기기로 알림이 계속 갑니다 | |
| 저장한 장소 · 좋아요(글·후기) · 차단 목록 | |
| 개인 플랜(남은 멤버가 없는 플랜) | 공유 플랜 → 소유권 이전 |

- 닉네임은 **`탈퇴한 사용자`**로 바뀝니다(`authors.nickname`과 `reviews.author_nm` 양쪽)
- **공유 플랜은 가장 먼저 합류한 편집자에게 소유권이 넘어갑니다.** 아무도 없으면 삭제됩니다
- **같은 이메일로 재가입할 수 있습니다** — 이메일을 지우므로 새 계정이 만들어집니다
- 같은 소셜 계정으로 다시 로그인해도 **새 계정**입니다(신원 행을 지웠으므로 `(provider, subject)`를 찾지 못합니다)
- ⚠️ 애플 로그인으로 가입한 계정은 삭제 시 서버가 애플에 **토큰 폐기**를 요청합니다(심사 규칙 4.8). 폐기에 실패해도 삭제는 진행합니다 — 폐기 실패로 탈퇴를 막는 것이 더 나쁩니다

#### 애플 로그인에 붙는 것
`POST /v1/auth/apple`에 **`authorizationCode`를 함께 보내면** 서버가 refresh token을 받아 두고,
계정 삭제 때 그 토큰으로 폐기를 호출합니다. 안 보내도 로그인은 그대로 되지만 폐기는 건너뜁니다.

### 차단
```
GET    /v1/blocks              🔒 → { count, items: [{ uuid, nickname, avatarUrl }] }
POST   /v1/blocks  { uuid }    🔒 → 204   (재차단 멱등 · 자기 차단은 400 · 없는 사람 404)
DELETE /v1/blocks/:uuid        🔒 → 204   (없는 것도 204)
```

**단방향 + 쓰기 차단입니다.**
① 내가 차단한 사람의 글·후기·댓글이 **내 목록에서 사라지고** 알림도 오지 않습니다.
② 그 사람은 **내 글·후기에 새 댓글을 달 수 없습니다**(403 `blocked_by_author`).
상대 화면에서 내 글이 사라지지는 않습니다.

걸러지는 응답: `GET /v1/posts`(홈·저장·장소별 전부), `GET /v1/posts/:id/comments`,
`GET /v1/reviews`, `GET /v1/reviews/:id/comments`, `GET /v1/reviews/summary`(평균·개수·태그 집계),
그리고 푸시 알림.

- **`total`과 `count`가 함께 줄어듭니다** — 어긋나면 페이지 개수가 맞지 않습니다
- 비로그인 응답은 아무것도 걸러지지 않습니다(차단은 로그인한 사람의 설정입니다)
- `GET /v1/barrier-free/:contentId`는 후기를 담지 않습니다 — 장소 화면의 후기는 위 두 라우트로 오므로 함께 걸러집니다

### authorInfo.uuid
게시글·후기·양쪽 댓글의 `authorInfo`에 `uuid`가 실립니다 — 차단·신고가 가리킬 식별자입니다.
닉네임으로 차단하면 동명이인이 함께 차단됩니다. 레거시 후기(작성자 계정이 없는 행)는 `null`이라
앱이 차단 메뉴를 감춰야 합니다.

## 신고

### POST /v1/reports — 신고  🔒
게시글·댓글·후기를 한 라우트로 신고합니다. 성공 시 **204**(본문 없음).

```jsonc
{ "targetType": "post",          // post | post_comment | review | review_comment
  "targetId": "6194e26…",        // 게시글은 uuid, 나머지는 정수 — 모두 문자열로 보냅니다
  "reason": "spam",              // spam | abuse | falseInfo | privacyLeak | irrelevant | other
  "detail": "광고 링크가 있어요" }  // 선택, 300자에서 잘립니다
```

| 상황 | 응답 |
|---|---|
| 정상 | 204 |
| 같은 사람의 재신고 | **204** — 새 행이 아니라 갱신입니다(멱등) |
| 자기 것 신고 | **204** — 기록하지 않고 무시합니다(아래 참고) |
| 모르는 `targetType` | 400 `invalid_target_type` |
| 화이트리스트에 없는 `reason` | 400 `invalid_reason` |
| 없는 대상·형식이 틀린 `targetId` | 404 |
| 비로그인 | 401 `login_required` |

- ⚠️ **신고가 대상을 감추지 않습니다.** 처리는 운영 판단입니다. 자동으로 숨기면 신고 버튼이 남의 글을 지우는 도구가 됩니다
- **자기 것 신고는 204로 무시합니다.** 오류가 아닌 이유는 앱이 오류창을 띄울 이유가 없기 때문이고, 기록하지 않는 이유는 자기 신고가 운영 신호가 아니라 잡음이며 자기 글을 반복 신고해 "신고 많은 대상" 통계를 밀어 올리는 길이 되기 때문입니다
- 대상이 지워져도 **신고 기록은 남습니다.** "신고된 뒤 지워졌다"는 운영에 필요한 사실입니다. 신고가 있어도 작성자는 자기 글을 지울 수 있습니다
- 신고자가 계정을 지우면 그 사람의 신고는 함께 지워집니다

### 신고 운영 화면 (대시보드)
`https://moduwa.app/dashboard` → **신고** 탭. 비밀번호 로그인 뒤에 있고, 앱이 아니라 웹입니다.

- 신고를 **대상별로 묶어** 보여줍니다 — "3명이 신고한 글"이 "1명이 신고한 글" 셋보다 먼저 보입니다
- **신고당한 본문과 작성자를 그 자리에서** 보여줍니다. `reports`에는 `target_id`만 있어서, 조인하지 않으면 신고를 볼 때마다 SQL을 쳐야 합니다
- 「문제 없음」·「조치함」으로 판단을 기록합니다(`resolved_at`, `resolved_note`). 되돌릴 수 있습니다
- 대상이 이미 지워졌으면 `이미 삭제됨`으로 표시합니다 — 신고 기록은 남습니다
- 탭에 미처리 건수 배지가 뜹니다(첫 진입에 바로 보입니다)
- ⚠️ **콘텐츠를 지우는 버튼은 없습니다.** 대시보드는 조회 전용이고, 파괴적 동작을 넣으면 비밀번호 하나로 남의 글을 지울 수 있게 됩니다. 실제 조치는 사람이 따로 하고 여기에는 판단만 남깁니다

### POST /v1/reviews/:reviewId/report — 후기 신고 (구 경로)  🔒
앱이 이미 쓰고 있어 남겨 둔 경로입니다. `POST /v1/reports`에 `targetType: "review"`로 보낸 것과
