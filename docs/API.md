...
구글·애플은 그렇고, 카카오는 확인 수단이 없어 절대 붙이지 않습니다(카카오는 같은 주소여도
별도 계정이 되고, 그 계정의 `email` 은 `null` 로 시작합니다). 카카오 사용자를 기존 계정과 잇는
길은 로그인한 뒤 설정에서 붙이는 것입니다(아래 "연결된 계정").
...
오류: `missing_idToken`(400) · `invalid_token`(401) · `social_not_configured`(503) · `too_many_attempts`(429) · `conflict`(409)

### GET · POST · DELETE /v1/auth/me/identities — 연결된 계정  🔒

로그인한 상태에서 다른 로그인 방식을 붙이고 뗍니다(설정 → 연결된 계정). 위의 자동 연결이
못 덮는 경우를 위한 것입니다 — **카카오**(이메일을 검증하지 않아 자동 연결 대상이 아님),
애플 "이메일 가리기"(주소가 다름), 이메일 계정에 구글을 나중에 붙이기. 근거는 이메일이 아니라
**지금 세션이 이 계정의 주인**이라는 사실이라 이메일이 달라도 붙습니다.
