...
별도 계정이 되고, 그 계정의 `email` 은 `null` 로 시작합니다).
...
- 애플의 "이메일 가리기"(비공개 릴레이 주소)는 기존 주소와 같을 수 없으므로 연결되지 않습니다.
  그 사람은 별도 계정이 됩니다. 로그인 상태에서 계정을 잇는 기능은 아직 없습니다(아래).
...
- **로그인 상태에서 계정 연결** — `POST /v1/auth/me/identities` 에 `idToken` 을 보내 지금 계정에
  다른 로그인 방식을 붙이는 것. 이메일이 달라 자동 연결이 안 되는 경우(카카오, 애플 이메일
  가리기)와 "이메일 계정에 구글을 나중에 붙이고 싶다"를 덮습니다.
