...
`AUDIT_API_KEYS`에 등록한 키는 **공공 관광 데이터만** 읽습니다. 공공데이터 활용 감사처럼
외부에 데이터 확인을 열어줄 때 쓰며, 일반 키(`API_KEYS`) 대신 이 키를 발급합니다.

| | |
|---|---|
| 열리는 것 | `/health` · `/v1/pet-friendly*` · `/v1/attractions*` · `/v1/barrier-free*` · `/v1/search` |
| 막히는 것 | 그 외 전부 → **403** `forbidden` |

- ⚠️ **일반 키를 외부에 주면 안 됩니다.** 그 키로는 `/v1/reviews`·`/v1/posts`가 열리고, 거기에는 닉네임·후기 본문·`authorInfo.uuid`가 실립니다. 후기 본문에는 이용자가 직접 적은 장애 정보(민감정보)가 있고, `uuid`로는 한 사람의 글을 전부 엮을 수 있습니다
- **차단 목록이 아니라 허용 목록입니다**(fail-closed). 새 라우트가 생기면 감사용 키에는 자동으로 닫힙니다 — 잊어서 개인정보가 새는 방향보다, 감사자가 403을 보고 문의하는 방향이 낫습니다
- 감사가 끝나면 `AUDIT_API_KEYS`에서 그 키를 지우면 즉시 폐기됩니다
