...
//
// ── 왜 라이브러리를 쓰지 않나
//  APNs 는 HTTP/2 + ES256 JWT 두 가지만 요구하고, 둘 다 Node 내장으로 된다(undici 의 fetch 가
//  HTTP/2 를 처리하고, crypto 가 ES256 을 서명한다). 인증 토큰은 최대 1시간 유효해 캐시하면
//  요청마다 서명할 일도 없다. 의존성을 늘리는 대신 100줄을 직접 두는 편이 낫다고 봤다.
//
...
export async function pushToAuthor(
  ...
  try {
    ...
    for (const d of devices) {
      ...
      try {
        const res = await fetch(`${GATEWAY[env]}/3/device/${d.token}`, {
          method: 'POST',
          headers: {
