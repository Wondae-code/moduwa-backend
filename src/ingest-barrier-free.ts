...
async function main() {
  ...
  // 28속성 중 하나라도 비어있지 않으면 접근성 정보 보유
  const hasAccessExpr = ATTRS.map((a) => `nullif(w.${a},'') is not null`).join(' or ');

  const res = await query(`
    insert into barrier_free (
      contentid, title, contenttypeid, addr1, addr2, mapx, mapy, firstimage, firstimage2,
      area_code, sigungu_code, ldong_regn_cd, ldong_signgu_cd, ${attrCols}, has_image, has_access)
    select
    ...
    on conflict (contentid) do update set
      ...
      ${attrUpd}, has_image = excluded.has_image, has_access = excluded.has_access, updated_at = now()
  `);
